...
Page({
    ...
    async onSubmit() {
        //保存照片
        let imageInfo = await wx.canvasToTempFilePath({
            canvasId: 'handWriting',
            fileType: 'jpg',
            quality: 1, //图片质量
        }).then(res => {
            ...
            if (res.errMsg == 'canvasToTempFilePath:ok') {
                ...
                wx.showToast({
                    title: '获取信息失败，请重新尝试' + res.errMsg,
                    icon: 'none'
                })
            }
            return imageInfo
        })
        console.log('onSubmit saveImage', imageInfo)
        if (!imageInfo.state) {
            return
        }
        //上传照片
        ...
        formData.appendFile("file", imageInfo.imageUrl);
            ...
            success: res => {
                ...
                if (!util.isEmpty(res) && res.statusCode == 200 && res.data.success == true) {
                    ...
                    if (!util.isEmpty(resData) && !util.isEmpty(resData.Id)) {
                        console.log('onSubmit up success url', `${app.config['resourceDownloadApi']}${resData.Id}`)
                        //返回签收页面
                        let pages = getCurrentPages();
                        let prePage = pages[pages.length - 2];
                        prePage.setData({
                            'parames.operate': OperateWritingEnum.submit,
                            'parames.writingSignId': resData.Id
                        }, () => {
                            wx.navigateBack()
                        })
