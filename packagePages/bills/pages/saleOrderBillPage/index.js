...
const saleBillService = require('../../../../services/saleBill.service')
const terminalService = require('../../../../services/terminal.service')
const saleReservationBillService = require('../../../../services/saleReservationBill.service')
...
import OperateWritingEnum from '../../../../models/operateWritingEnum'

//单据缓存KEY
const cacheKey = 'CACHE_SALERESERVATIONBILLADD'
...
Page({
    data: {
        ...
        menusConf: {},
        bill: {
            BillTypeId: BillTypeEnum.SaleReservationBill,
            MakeUserId: app.global.userId,
            MakeUserName: app.global.userInfo.userRealName,
            CreatedOnUtc: util.time(),
            BillNumber: util.getBillNumber(BillTypeEnum.properties[BillTypeEnum.SaleReservationBill].code, app.global.storeId ?? 0),
            WareHouseId: '0',
            TerminalId: 0,
            TerminalName: '',
            WareHouseName: '',
    ...
    onLoad: async function (options) {
        //console.log('*********** salebillpage on load ***********', options)
        ...
        if (!util.isEmpty(options.pageType) && parseInt(options.pageType) == PageTypeEnum.sign) {
            wx.getStorage({
                key: app.global.saleOrderBillSignStoragekey,
                success: res => {
                    console.log('saleOrderBillSignStoragekey', res)
                    this.setData({
                        saleOrderBillSignBill: res.data
                    })
                }
            })
        }
        ...
        await wareHousesService
            .getWareHousesAsync(BillTypeEnum.SaleReservationBill)
            .then(res => {
    ...
    async initBill(clear = false) {
        ...
        let bt = BillTypeEnum.SaleReservationBill
        let stb = app.global.saleReservationBill
        //临时bill
        let bill = {}
        bill.BillTypeId = bt
        ...
        bill.BusinessUserName = app.global.userInfo.UserRealName
        ...
        let resultCheck = await authenticationUtil.checkBusinessTime(BillTypeEnum.SaleReservationBill)
        this.setData({
        ...
        //编辑明细回传
        else if (pms.billType == bt && (pms.pageType == PageTypeEnum.edit || pms.pageType == PageTypeEnum.check || pms.pageType == PageTypeEnum.sign) && !clear) {
            removeMenuIds = removeMenuIds.concat([0])
            if (!util.isEmpty(pms.billId) && pms.billId > 0) {
                await saleReservationBillService.GetBillAsync(pms.billId).then((res) => {
                    //console.log('res:', res);
                    if (res.code >= 0) {
                        bill = res.data
    ...
    toPayment(sourceBill) {
        let accountings = sourceBill.SaleReservationBillAccountings.map(item => {
            return {
    ...
    cacheBill() {
        let key = ''
        if (this.data.parames.billType == BillTypeEnum.SaleReservationBill && this.data.parames.pageType == PageTypeEnum.add) {
    ...
    onShow: async function () {
        if (this.data.parames.pageType == PageTypeEnum.sign && this.data.parames.operate == OperateWritingEnum.submit && !util.isEmpty(this.data.parames.writingSignId)) {
            //单据签收
            let writingSignId = this.data.parames.writingSignId
            this.setData({
                'parames.operate': OperateWritingEnum.cancel,
                'parames.writingSignId': '',
                'data.bill.writingSignId': writingSignId
            }, () => {
                //提交单据签收
    ...
    initPayment(initPay = true) {
        saleReservationBillService.GetInitDataAsync(this.data.parames.billType).then(res => {
            //console.log('initPayment', res)
            if (!util.isEmpty(res)) {
                let defaultPayments = res.SaleReservationBillAccountings.map(item => {
                    return {
                        accountCodeTypeId: item.AccountCodeTypeId,
                        default: true,
                        name: item.Name,
                ...
                if (initPay) {
                    this.setData({
                        ...
                        'bill.paymentMethods.PreferentialAmount': 0,
                        saleDefaultAmounts: res.SaleDefaultAmounts
                    }, () => {
    ...
    onUnload: function () {
        ...
        // if (!util.isEmpty(inactive)) {
        //     printMain.myCloseBLEConnection(inactive)
        // }
    },
    ...
    onSelectTerminal: function (e) {
        ...
        wx.navigateTo({
            url: '/packagePages/common/pages/selectCustomerPage/index?type=select&navigatemark=/packagePages/bills/pages/saleOrderBillPage/index'
        });
    },
    onShowPopupMenus(e) {
        if (!this.data.billCanEdit) {
    ...
    async clickMenuItem(e) {
        ...
        switch (m.id) {
            ...
            case 0: {
                ...
                wx.navigateTo({
                    url: `/packagePages/order/pages/billSummaryPage/index?defaultPageKey=${BillTypeEnum.SaleReservationBill}&title=查看历史单据&terminalId=${this.data.bill.TerminalId}`
                });
                break;
            }
            //支付方式
            ...
            case 5:
                //冲改
            case 7: {
                if ((util.isEmpty(this.data.bill.items) || this.data.bill.items.length <= 0) && m.id != 1) {
                    wx.showToast({
                        title: '请先选择商品',
                        icon: 'error'
                    })
    ...
    onReverse() {
        if (!authenticationUtil.checkPermission(MenuEnum.SaleReservationBillReverse)) {
            wx.showToast({
                title: '您无权限冲改',
                icon: 'error',
                duration: 5000
        ...
        if (!util.isEmpty(this.data.bill.Id) && this.data.bill.Id > 0) {
            ...
            saleReservationBillService.ReverseAsync(this.data.bill.Id, remark).then((res) => {
                //console.log('res:', res);
    ...
    onAuditing() {
        if (!authenticationUtil.checkPermission(MenuEnum.SaleReservationBillApproved)) {
            wx.showToast({
                title: '您无权限审核',
                icon: 'error',
                duration: 5000
        ...
        if (!util.isEmpty(this.data.bill.Id) && this.data.bill.Id > 0) {
            saleReservationBillService.AuditingAsync(this.data.bill.Id).then((res) => {
                //console.log('res:', res);
                if (res.code >= 0) {
                    wx.showToast({
                        title: '审核成功',
    ...
    async onSubmit(isPrint = false) {
        ...
        if (this.data.terminalBalance.MaxOweCashBalance != 0) {
            let totalOweAndBill = (parseFloat(this.data.terminalBalance.TotalOweCash) + parseFloat(bill.paymentMethods.OweCash))
            //console.log('欠', totalOweAndBill)
            if (this.data.terminalBalance.MaxOweCashBalance <= totalOweAndBill) {
                wx.showToast({
                    // title: `本次开单欠款总额达${totalOweAndBill}超过最大欠款额${this.data.terminalBalance.MaxOweCashBalance}`,
                    title: `欠款金额已超，该终端最大欠款为${this.data.terminalBalance.MaxOweCashBalance}元`,
                    icon: 'none',
                    duration: 5000
                })
                return false
        ...
        postMData.orderId = bill.SaleReservationBillId ?? 0
        ...
        let result = await saleReservationBillService.CreateOrUpdateAsync(postMData, postMData.id).then(res => {
    ...
    onMorePayment: function () {
        ...
        wx.navigateTo({
            url: '/packagePages/common/pages/morePaymentPage/index?billTypeId=' + BillTypeEnum.SaleReservationBill
        });
    },
    //提交打印
    async onSubmitPrint() {
    ...
    onPrint: async function () {
        ...
        //新方法打印 判断蓝牙状态并连接
        await printMain.printBill(inactive, BillTypeEnum.SaleReservationBill, billData)
        return true
    },
    onSignature() {
        wx.navigateTo({
            url: '/packagePages/common/pages/writingSignature/index',
        })
    },
    async submitSignature() {
        let bill = this.data.bill
        let saleOrderBillSignBill = this.data.saleOrderBillSignBill
        var postMData = {}
        postMData.id = 0
        postMData.billNumber = util.getBillNumber(BillTypeEnum.properties[BillTypeEnum.SaleBill].code, app.global.storeId ?? 0)
        postMData.orderId = bill.Id ?? 0
        postMData.terminalId = bill.TerminalId
        postMData.businessUserId = bill.BusinessUserId
        postMData.deliveryUserId = bill.DeliveryUserId //app.global.userInfo.Id
        postMData.wareHouseId = bill.WareHouseId ?? 0
        postMData.districtId = bill.DistrictId ?? 0
        postMData.payTypeId = bill.PayTypeId ?? 0
        postMData.defaultAmountId = bill.DefaultAmountId
        postMData.sumAmount = bill.SumAmount
        postMData.receivableAmount = bill.ReceivableAmount
        postMData.preferentialAmount = bill.paymentMethods.PreferentialAmount
        postMData.oweCash = bill.paymentMethods.OweCash
        postMData.remark = bill.Remark ?? ''
        postMData.auditedStatus = false
        postMData.reversedStatus = bill.ReversedStatus
        postMData.items = bill.items
        postMData.dispatchItemId = saleOrderBillSignBill.Id
        postMData.longitude = app.global.bd09_location.lon
        postMData.latitude = app.global.bd09_location.lat
        ...
        //签名文件图片ID
        postMData.signature = bill.writingSignId

        console.log('postData', postMData)
        let result = await saleBillService.CreateOrUpdateAsync(postMData, postMData.id).then(res => {
            console.log('CreateOrUpdateAsync', res)
            if (res.success == true && res.data.Success == true) {
                wx.showToast({
                    title: '提交成功',
