...
//单据缓存KEY
const cacheKey = 'CACHE_SALEBILLADD'
...
Page({
    data: {
        ...
        menusConf: {},
        bill: {
            BillTypeId: BillTypeEnum.SaleBill,
            MakeUserId: app.global.userId,
            MakeUserName: app.global.userInfo.userRealName,
            CreatedOnUtc: util.time(),
            BillNumber: util.getBillNumber(BillTypeEnum.properties[BillTypeEnum.SaleBill].code, app.global.storeId ?? 0),
            WareHouseId: '0',
            TerminalId: 0,
            TerminalName: '',
            WareHouseName: '',
    ...
    onLoad: async function (options) {
        ...
        await wareHousesService
            .getWareHousesAsync(BillTypeEnum.SaleBill)
            .then(res => {
    ...
    async initBill(clear = false) {
        ...
        let bt = BillTypeEnum.SaleBill
        let stb = app.global.saleBill
        //临时bill
        let bill = {}
        bill.BillTypeId = bt
        ...
        let resultCheck = await authenticationUtil.checkBusinessTime(BillTypeEnum.SaleBill)
        this.setData({
        ...
        //编辑明细回传
            ...
            if (!util.isEmpty(pms.billId) && pms.billId > 0) {
                await saleBillService.GetBillAsync(pms.billId).then((res) => {
                    //console.log('res:', res);
                    if (res.code >= 0) {
                        bill = res.data
    ...
    toPayment(sourceBill) {
        let accountings = sourceBill.SaleBillAccountings.map(item => {
            return {
    ...
    cacheBill() {
        let key = ''
        if (this.data.parames.billType == BillTypeEnum.SaleBill && this.data.parames.pageType == PageTypeEnum.add) {
    ...
    initPayment(initPay = true) {
        saleBillService.GetInitDataAsync(this.data.parames.billType).then(res => {
            //console.log('initPayment', res)
            if (!util.isEmpty(res)) {
                let defaultPayments = res.SaleBillAccountings.map(item => {
                    return {
                        accountCodeTypeId: item.AccountCodeTypeId,
                        default: true,
                        name: item.Name,
    ...
    async clickMenuItem(e) {
        ...
        switch (m.id) {
            ...
            case 0: {
                ...
                wx.navigateTo({
                    url: `/packagePages/order/pages/billSummaryPage/index?defaultPageKey=${BillTypeEnum.SaleBill}&title=查看历史单据&terminalId=${this.data.bill.TerminalId}`
                });
                break;
            }
            //支付方式
    ...
    onReverse() {
        if (!authenticationUtil.checkPermission(MenuEnum.SaleBillReverse)) {
            wx.showToast({
                title: '您无权限冲改',
                icon: 'error',
                duration: 5000
        ...
        if (!util.isEmpty(this.data.bill.Id) && this.data.bill.Id > 0) {
            ...
            saleBillService.ReverseAsync(this.data.bill.Id, remark).then((res) => {
                //console.log('res:', res);
    ...
    onAuditing() {
        if (!authenticationUtil.checkPermission(MenuEnum.SaleBillApproved)) {
            wx.showToast({
                title: '您无权限审核',
                icon: 'error',
                duration: 5000
        ...
        if (!util.isEmpty(this.data.bill.Id) && this.data.bill.Id > 0) {
            saleBillService.AuditingAsync(this.data.bill.Id).then((res) => {
                //console.log('res:', res);
                if (res.code >= 0) {
                    wx.showToast({
                        title: '审核成功',
    ...
    onMorePayment: function () {
        ...
        wx.navigateTo({
            url: '/packagePages/common/pages/morePaymentPage/index?billTypeId=' + BillTypeEnum.SaleBill
        });
    },
    //提交打印
    async onSubmitPrint() {
    ...
    onPrint: async function () {
        ...
        //新方法打印 判断蓝牙状态并连接
        await printMain.printBill(inactive, BillTypeEnum.SaleBill, billData)
        return true
    },
})